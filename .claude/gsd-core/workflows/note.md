...
- **Global scope**: `/home/user/R6_Manager/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/user/R6_Manager/.claude/notes/`)
...
2. Glob `/home/user/R6_Manager/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/user/R6_Manager/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/user/R6_Manager/.claude/notes/` — works in any directory
